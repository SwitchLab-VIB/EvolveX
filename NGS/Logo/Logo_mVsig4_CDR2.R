...
mR1NC_CDR2 <- "mR1NC_CDR2.csv"
WT <- "WT.csv"
mR1_CDR2 <- "mR1_CDR2.csv"
input <-"input.csv"


# Load the CSV file
data_wt <- read.csv(WT)
data_mR1_CDR2 <- read.csv(mR1_CDR2)
data_mR1NC_CDR2 <- read.csv(mR1NC_CDR2)
...
p1 <- ggseqlogo(data_mR1_CDR2$Sequence, method = 'prob',col_scheme = csRasmol) +
  scale_x_continuous(breaks = 1:10, labels = 50:59, limits = x_limits) +
  theme(legend.position = "none",
        axis.title.x = element_text(vjust = -0.5, hjust = 0.28)) +
  labs(x = "Position", title = "Mouse Vsig4 R1 31 enriched designs, CDR2") +
  theme(plot.title = element_text(size = 14), axis.text.x = element_text(size = 8))

p2 <- ggseqlogo(data_mR1NC_CDR2$Sequence, method = 'prob',col_scheme = csRasmol) +
  scale_x_continuous(breaks = 1:10, labels = 50:59, limits = x_limits) +
  theme(legend.position = "none",
        axis.title.x = element_text(vjust = -0.5, hjust = 0.28)) +
  labs(x = "Position", title = "Mouse Vsig4 R1 31 negative designs, CDR2") +
  theme(plot.title = element_text(size = 14), axis.text.x = element_text(size = 8))
...
Mouse_sequence_logos_CDR2 <- plot_grid(p3, p4, p1, p2, ncol = 1)

# Display the Mouse sequence logos
print(Mouse_sequence_logos_CDR2)

ggsave("Mouse_sequence_logos_CDR2.pdf", Mouse_sequence_logos_CDR2, dpi = 600, width = 8, height = 6)
