# ===============================
# List of experiments for mVsig4 CDR2
...
# Open combined PDF
pdf("All_mVsig4_CDR2_plots.pdf", width = 8, height = 6)

for (exp_name in exp_list) {
  
  # Read CSV
  file_name <- paste0("m", exp_name, "_CDR2.csv")
  ...
  # Apply color and shape mappings (mVsig4 CDR2-specific Hits)
  data$color_mapping <- sapply(data$ID, get_color)
  data$color_mapping <- factor(data$color_mapping, levels = color_levels)
  
  data$shape_mapping <- ifelse(data$ID %in% c("Rob_6729", "Vsig4_WT"),
                               "Hits", "others")
  data$shape_mapping <- factor(data$shape_mapping, levels = shape_levels)
  
  # Label mapping (mVsig4 CDR2-specific)
  data$label_mapping <- ifelse(data$ID == "Rob_6729", "VHH_m1_2_3_4",
  ...
  plot <- ggplot(data, aes(x = Enrichment_factor, y = Count_binding)) +
    ...
    labs(color = "Point Color",
         title = paste("Mouse Vsig4", exp_name, "enriched CDR2")) +
