...
# Open combined PDF
pdf("All_hVsig4_CDR3_plots.pdf", width = 8, height = 6)

for (exp_name in exp_list) {
  
  # Read CSV
  file_name <- paste0("h", exp_name, "_CDR3.csv")
  ...
  # Apply color and shape mappings (CDR3-specific Hits)
  data$color_mapping <- sapply(data$ID, get_color)
  data$color_mapping <- factor(data$color_mapping, levels = color_levels)
  
  data$shape_mapping <- ifelse(data$ID %in% c("Rob_7859", "Rob_7571", "Rob_7861", "Vsig4_WT"),
                               "Hits", "others")
  data$shape_mapping <- factor(data$shape_mapping, levels = shape_levels)
  
  # Label mapping (CDR3-specific)
  data$label_mapping <- ifelse(data$ID == "Rob_7859", "VHH_h1_2_5",
                               ifelse(data$ID == "Rob_7571", "VHH_h3",
                                      ifelse(data$ID == "Rob_7861", "VHH_h4",
  ...
  plot <- ggplot(data, aes(x = Enrichment_factor, y = Count_binding)) +
    ...
    labs(color = "Point Color",
         title = paste("Human Vsig4", exp_name, "enriched CDR3")) +
